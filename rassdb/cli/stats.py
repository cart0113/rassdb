...
def format_stats(stats: Dict[str, Any], format: str) -> str:
    ...
    if format == "json":
        ...
        if stats.get("by_language"):
            lang_data = [
                [lang, count] for lang, count in sorted(stats["by_language"].items())
            ]
            output.append("Chunks by Language:")
            output.append(
                tabulate(lang_data, headers=["Language", "Count"], tablefmt="grid")
            )
            output.append("")
        ...
        if stats.get("by_type"):
            ...
            output.append(
                tabulate(type_data, headers=["Type", "Count"], tablefmt="grid")
            )
